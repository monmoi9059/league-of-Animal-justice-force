...
export class CaptainEnemy extends Enemy {
    ...
    draw(ctx, camX, camY, now) {
        ...
        // GAMER CAPTAIN
        // Fat Body
        ctx.fillStyle = "#333"; // Dark hoodie
        // Bigger body than normal
        drawRoundedRect(ctx, cx, cy + 10, 50, 50, 10);

        // Head
        ctx.fillStyle = "#FFE4C4"; // Pale skin
        drawRoundedRect(ctx, cx + 10, cy - 10, 30, 30, 8);

        // Headset
        ctx.strokeStyle = "#111"; ctx.lineWidth = 3;
        ctx.beginPath(); ctx.arc(cx + 25, cy - 5, 18, Math.PI, 0); ctx.stroke(); // Band
        ctx.fillStyle = "#222";
        ctx.fillRect(cx + 5, cy - 8, 8, 12); ctx.fillRect(cx + 37, cy - 8, 8, 12); // Cans
        // Mic
        ctx.strokeStyle = "#333"; ctx.lineWidth = 2;
        ctx.beginPath(); ctx.moveTo(cx+37, cy); ctx.lineTo(cx+42, cy+5); ctx.lineTo(cx+30, cy+10); ctx.stroke();
        ctx.fillStyle = "#555"; ctx.beginPath(); ctx.arc(cx+30, cy+10, 2, 0, Math.PI*2); ctx.fill();

        // Eyes (Tired)
        ctx.fillStyle = "#000";
        ctx.beginPath(); ctx.arc(cx + 20, cy, 2, 0, Math.PI*2); ctx.fill();
        ctx.beginPath(); ctx.arc(cx + 30, cy, 2, 0, Math.PI*2); ctx.fill();
        // Bags under eyes
        ctx.strokeStyle = "rgba(0,0,0,0.2)"; ctx.lineWidth=1;
        ctx.beginPath(); ctx.arc(cx+20, cy+2, 3, 0, Math.PI); ctx.stroke();
        ctx.beginPath(); ctx.arc(cx+30, cy+2, 3, 0, Math.PI); ctx.stroke();

        // Controller in hands
        ctx.fillStyle = "#222";
        ctx.save();
        ctx.translate(cx + 25, cy + 40);
        // Hands holding it
        ...
        // Controller body
        ctx.fillStyle = "#444";
        drawRoundedRect(ctx, -12, -4, 24, 12, 4);
        ctx.restore();
    }
}
