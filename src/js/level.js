            ...
            if (beastsPlaced < 8 && (x - lastBeastX > 30) && secureRandom() < 0.08) {
                if (secureRandom() < 0.5 && currentHeight > 6) {
                    ...
                    if (islandY > 2) {
                        ...
                        // Guard Squad on the island or below?
                        // Let's spawn a specialist ON the island
                        if (difficulty >= 3) newEntities.push(new ShieldBearer((x+1) * TILE_SIZE, (islandY) * TILE_SIZE));
                        else newEntities.push(new Enemy((x+1) * TILE_SIZE, (islandY) * TILE_SIZE));

                        // And a squad below to intercept player
                        spawnSquad(x, currentHeight-1);
                    }
                }
            }
        }
