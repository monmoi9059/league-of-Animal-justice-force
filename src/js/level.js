...
function generateLevel() {
            ...
            for (let i = 0; i < numTunnels; i++) {
                ...
                for (let j = 0; j < tunnelLen; j++) {
                    ...
                    if (secureRandom() < 0.05 && j > 5) {
                        ...
                        if (rand < 0.6) newEntities.push(new Enemy(tx * TILE_SIZE, (tunnelY + 2) * TILE_SIZE));
                        else newEntities.push(new KamikazeEnemy(tx * TILE_SIZE, (tunnelY + 2) * TILE_SIZE));
                    }

                    // Falling Block Trap (Ceiling)
    ...
    for (let x = 20; x < LEVEL_WIDTH - 40; x++) {
        ...
        else if (secureRandom() < 0.06) {
             // More varied enemies
             let r = secureRandom();
             if (r < 0.5) newEntities.push(new Enemy(x * TILE_SIZE, (y-1) * TILE_SIZE));
             else if (r < 0.8) newEntities.push(new KamikazeEnemy(x * TILE_SIZE, (y-1) * TILE_SIZE));
             else newEntities.push(new HeavyGunner(x * TILE_SIZE, (y-1) * TILE_SIZE - 10)); // Slightly taller
        }
